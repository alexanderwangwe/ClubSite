...
import { ArrowLeft, Users, Heart, Recycle, Mail, Phone, MessageCircle } from "lucide-react"
      ...
      <section className="bg-gradient-to-r from-primary/10 to-accent/10 py-16">
        ...
        <section className="py-20 bg-white">
          <div className="max-w-6xl mx-auto px-4 text-center">
            <h2 className="text-3xl font-bold text-green-800 mb-12">Ways to Participate</h2>
            <div className="grid md:grid-cols-3 gap-8">
              {participationWays.map((way, index) => (
                <Card
                  key={index}
                  className="border-green-200 hover:shadow-lg hover:-translate-y-1 transition transform"
                >
                  <CardHeader className="flex flex-col items-center">
                    {way.icon}
                    <CardTitle className="mt-4 text-green-800 text-xl">{way.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="text-gray-600">{way.description}</CardContent>
                </Card>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-green-50">
          <div className="max-w-4xl mx-auto text-center px-4">
            <h2 className="text-3xl font-bold text-green-800 mb-10">Get in Touch</h2>
        ...
        <section className="py-20 bg-white">
          <div className="max-w-3xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-green-800 mb-8 text-center">FAQ</h2>
            <Card className="p-6 border-green-200 shadow-md">
              <Accordion type="single" collapsible>
                {faqs.map((faq, index) => (
                  ...
                  >
                    <AccordionTrigger className="text-green-800 text-lg">{faq.question}</AccordionTrigger>
